...
export default class Database {
  constructor() {
    ...
    this.deviceSchema = new mongoose.Schema({
      ...
      device_name: {
        type: String,
        required: true,
        default: "Inget namn specifierat",
        unique: true,
        dropDups: true,
      },
      token: { type: String, unique: true, dropDups: true },
    ...
    this.models = {
      ...
      henrikjonssonlogs: check_if_model_defined(
        "henrikjonssonlogs",
        this.logsSchema
      ),
      sarahhagberglogs: check_if_model_defined(
        "sarahhagberglogs",
        this.logsSchema
      ),
      vincentperssonlogs: check_if_model_defined(
        "vincentperssonlogs",
        this.logsSchema
      ),
      megansundströmlogs: check_if_model_defined(
        "megansundströmlogs",
        this.logsSchema
      ),
      mathiaslavenologs: check_if_model_defined(
        "mathiaslavenologs",
        this.logsSchema
      ),
  ...
  add_device(device_name, user_id, token) {
    ...
    return new Promise((resolve, reject) => {
      this.models.devices.create(
        { device_name: device_name, user: user_id, token: token },
        (err, result) => {
          if (err) reject(err);
          else resolve(result);
        }
  ...
  add_logs(collection_name, logs) {
    try {
      return new Promise((resolve, reject) => {
        this.models[collection_name].create({ logs: logs }, (err, res) => {
          if (err) reject(err);
          else resolve(res);
        });
      });
  ...
  print_uris() {
    this.models.users.find({}, (err, result) => {
      result.forEach((person) => {
        console.log(
          `${person.name} ${person.role} -> ${process.env.HOST_URL}setstatus?auth=${person.uri}`
        );
      });
    });
  }
